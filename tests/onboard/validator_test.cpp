...
TEST(ValidatorTest, CreateWithNullKey) {
    sum2_device_id_t dev = {};
    auto *v = sum2_validator_create(nullptr, 0, &dev);
    ASSERT_NE(v, nullptr);
...
TEST(ValidatorTest, CreateWithNullDeviceId) {
    uint8_t key[32] = {};
    auto *v = sum2_validator_create(key, sizeof(key), nullptr);
    ASSERT_NE(v, nullptr);
    sum2_validator_free(v);
}
...
TEST(ValidatorTest, CreateWithCoseKey) {
    sum2_device_id_t dev = {};
    auto *v = sum2_validator_create(kTrustAnchorCoseKey,
...
TEST(ValidatorTest, FreeNull) {
    sum2_validator_free(nullptr);  // Should not crash
}
...
TEST(ValidatorTest, AddMultipleTrustAnchors) {
    ...
    int rc = sum2_validator_add_trust_anchor(v, kTrustAnchorCoseKey,
...
TEST(ValidatorTest, RevokeKid) {
    ...
    int rc = sum2_validator_revoke_kid(v, kid, sizeof(kid));
    EXPECT_EQ(rc, SUM2_OK);
...
TEST(ValidatorTest, RevokeKidRejectsNull) {
    auto *v = sum2_validator_create(nullptr, 0, nullptr);
    ASSERT_NE(v, nullptr);

    EXPECT_NE(sum2_validator_revoke_kid(v, nullptr, 0), SUM2_OK);
    EXPECT_NE(sum2_validator_revoke_kid(v, nullptr, 5), SUM2_OK);

    uint8_t kid[] = {0x01};
    EXPECT_NE(sum2_validator_revoke_kid(v, kid, 0), SUM2_OK);
...
protected:
    void SetUp() override {
        ...
        sum2_device_id_t dev = {};
        v_ = sum2_validator_create(kTrustAnchorCoseKey,
                                    sizeof(kTrustAnchorCoseKey), &dev);
        ASSERT_NE(v_, nullptr);
    }

    void TearDown() override {
        sum2_validator_free(v_);
    }

    sum2_validator_t *v_ = nullptr;
    std::vector<uint8_t> envelope_;
};

TEST_F(EnvelopeTest, DecodeValidEnvelope) {
    sum2_manifest_t *m = nullptr;
    int rc = sum2_validate_envelope(v_, envelope_.data(), envelope_.size(),
                                     0, &m);
    EXPECT_EQ(rc, SUM2_OK) << "Failed to validate example 0 envelope";

    if (m) {
        /* Example 0 has sequence_number = 0, 1 component, no dependencies */
        EXPECT_EQ(sum2_manifest_sequence_number(m), 0u);
        EXPECT_GE(sum2_manifest_component_count(m), 1u);
        EXPECT_EQ(sum2_manifest_is_campaign(m), 0);
...
TEST_F(EnvelopeTest, RejectGarbageEnvelope) {
    ...
    sum2_manifest_t *m = nullptr;
    int rc = sum2_validate_envelope(v_, garbage, sizeof(garbage), 0, &m);
    EXPECT_NE(rc, SUM2_OK);
    if (m) sum2_manifest_free(m);
}

TEST_F(EnvelopeTest, RejectEmptyEnvelope) {
    sum2_manifest_t *m = nullptr;
    int rc = sum2_validate_envelope(v_, nullptr, 0, 0, &m);
    EXPECT_NE(rc, SUM2_OK);
}
...
TEST_F(EnvelopeTest, RejectTruncatedEnvelope) {
    ...
    sum2_manifest_t *m = nullptr;
    int rc = sum2_validate_envelope(v_, envelope_.data(), half, 0, &m);
    EXPECT_NE(rc, SUM2_OK);
    if (m) sum2_manifest_free(m);
}
...
TEST_F(EnvelopeTest, RejectWrongKey) {
    ...
    sum2_device_id_t dev = {};
    auto *v2 = sum2_validator_create(wrong_key, sizeof(wrong_key), &dev);
    ASSERT_NE(v2, nullptr);

    sum2_manifest_t *m = nullptr;
    int rc = sum2_validate_envelope(v2, envelope_.data(), envelope_.size(),
                                     0, &m);
    EXPECT_NE(rc, SUM2_OK);
    if (m) sum2_manifest_free(m);
...
TEST_F(EnvelopeTest, RejectTamperedSignature) {
    ...
    sum2_manifest_t *m = nullptr;
    int rc = sum2_validate_envelope(v_, tampered.data(), tampered.size(),
                                     0, &m);
    EXPECT_NE(rc, SUM2_OK);
    if (m) sum2_manifest_free(m);
}
...
TEST_F(EnvelopeTest, RejectTamperedManifest) {
    ...
    int rc = sum2_validate_envelope(v_, tampered.data(), tampered.size(),
                                     0, &m);
    EXPECT_NE(rc, SUM2_OK) << "Tampered manifest body should fail digest check";
    if (m) sum2_manifest_free(m);
}
...
TEST_F(EnvelopeTest, RollbackRejectsEqual) {
    ...
    sum2_validator_set_min_sequence(v_, nullptr, 0, 0);
...
TEST_F(EnvelopeTest, RollbackRejectsOlder) {
    ...
    sum2_validator_set_min_sequence(v_, nullptr, 0, 5);
...
TEST_F(EnvelopeTest, RollbackAcceptsNewer) {
    ...
    sum2_manifest_t *m = nullptr;
    int rc = sum2_validate_envelope(v_, env1.data(), env1.size(), 0, &m);
    EXPECT_EQ(rc, SUM2_OK) << "seq=1 should be accepted when min_seq=0";
    if (m) {
        EXPECT_EQ(sum2_manifest_sequence_number(m), 1u);
        sum2_manifest_free(m);
    }
}
...
TEST_F(EnvelopeTest, RevocationRejection) {
    ...
    sum2_validator_set_reject_before(v_, 2000000000);

    sum2_manifest_t *m = nullptr;
    int rc = sum2_validate_envelope(v_, env1.data(), env1.size(),
                                     1700000000, &m);
...
TEST_F(EnvelopeTest, RevocationAcceptAfter) {
    ...
    sum2_validator_set_reject_before(v_, 1700000000);
    ...
    if (rc == SUM2_OK && m) {
        EXPECT_GT(sum2_manifest_sequence_number(m), 0u);
        sum2_manifest_free(m);
    }
}
...
TEST_F(EnvelopeTest, RevokedKidRejectsManifest) {
    ...
    sum2_validator_revoke_kid(v_, kTrustAnchorKid, sizeof(kTrustAnchorKid));
...
TEST_F(EnvelopeTest, UnrelatedKidDoesNotBlock) {
    ...
    sum2_validator_revoke_kid(v_, other_kid, sizeof(other_kid));
...
TEST(ManifestAccessorTest, NullManifest) {
    EXPECT_EQ(sum2_manifest_sequence_number(nullptr), 0u);
    EXPECT_EQ(sum2_manifest_component_count(nullptr), 0u);
    EXPECT_EQ(sum2_manifest_dependency_count(nullptr), 0u);
    EXPECT_EQ(sum2_manifest_is_campaign(nullptr), 0);
}

TEST(ManifestAccessorTest, FreeNull) {
    sum2_manifest_free(nullptr);  // Should not crash
}
...
TEST(ManifestAccessorTest, ComponentIdOutOfBounds) {
    ...
    size_t out_len = 0;
    EXPECT_NE(sum2_manifest_component_id(nullptr, 0, &out, &out_len), SUM2_OK);
}
...
TEST_F(EnvelopeTest, ExtractVendorId) {
    ...
    int rc = sum2_validate_envelope(v_, envelope_.data(), envelope_.size(),
                                     0, &m);
    ASSERT_EQ(rc, SUM2_OK);
    ASSERT_NE(m, nullptr);

    /* exp0 vendor-id: fa6b4a53-d5ad-5fdf-be9d-e663e4d41ffe */
    uint8_t vid[16];
    rc = sum2_manifest_vendor_id(m, 0, vid);
    EXPECT_EQ(rc, SUM2_OK);
    if (rc == SUM2_OK) {
...
TEST_F(EnvelopeTest, ExtractClassId) {
    ...
    uint8_t cid[16];
    rc = sum2_manifest_class_id(m, 0, cid);
    EXPECT_EQ(rc, SUM2_OK);
    if (rc == SUM2_OK) {
        const uint8_t expected[] = {
...
TEST_F(EnvelopeTest, DeviceIdNotPresent) {
    ...
    uint8_t did[16];
    rc = sum2_manifest_device_id(m, 0, did);
    EXPECT_EQ(rc, SUM2_ERR_UNSUPPORTED);
    sum2_manifest_free(m);
}
...
TEST(ManifestAccessorTest, VersionNullManifest) {
    sum2_version_cmp_t cmp;
    int64_t parts[4];
    size_t parts_len = 4;
    EXPECT_NE(sum2_manifest_version(nullptr, 0, &cmp, parts, &parts_len),
              SUM2_OK);
}
...
TEST_F(EnvelopeTest, VersionNotPresent) {
    ...
    int64_t parts[4];
    size_t parts_len = 4;
    rc = sum2_manifest_version(m, 0, &cmp, parts, &parts_len);
    EXPECT_EQ(rc, SUM2_ERR_UNSUPPORTED);
    sum2_manifest_free(m);
}
...
TEST(ManifestAccessorTest, TextNullManifest) {
    const char *out;
    size_t out_len;
    EXPECT_NE(sum2_manifest_text_vendor_name(nullptr, 0, &out, &out_len),
              SUM2_OK);
    EXPECT_NE(sum2_manifest_text_model_name(nullptr, 0, &out, &out_len),
              SUM2_OK);
    EXPECT_NE(sum2_manifest_text_description(nullptr, &out, &out_len),
              SUM2_OK);
}
...
TEST_F(EnvelopeTest, TextNotPresent) {
    ...
    const char *out;
    size_t out_len;
    EXPECT_EQ(sum2_manifest_text_vendor_name(m, 0, &out, &out_len),
              SUM2_ERR_UNSUPPORTED);
    EXPECT_EQ(sum2_manifest_text_model_name(m, 0, &out, &out_len),
              SUM2_ERR_UNSUPPORTED);
    EXPECT_EQ(sum2_manifest_text_description(m, &out, &out_len),
              SUM2_ERR_UNSUPPORTED);
    sum2_manifest_free(m);
}
...
TEST_F(EnvelopeTest, TextFromExp2) {
    ...
    sum2_validator_set_min_sequence(v_, nullptr, 0, 1);
    int rc = sum2_validate_envelope(v_, env2.data(), env2.size(), 0, &m);
    if (rc != SUM2_OK) {
    ...
    const char *desc;
    size_t desc_len;
    rc = sum2_manifest_text_description(m, &desc, &desc_len);
    if (rc == SUM2_OK) {
